...
{
    public partial class HigherThanAvgForm : Form
    {
        SqlConnection connection;
        public HigherThanAvgForm(SqlConnection connection)
        {
            InitializeComponent();
...
JOIN [EA_type] t
ON t.[ID] = pl.[Type_ID]
where sl.[Price]>(select AVG(sl.[Price]) from [Product list] pl 
JOIN [Sales log] sl
ON pl.[ID] = sl.[Product_ID] 
JOIN [Manufacturers] m
ON m.[ID]=pl.[Manufacturers_ID]
where m.[Name]='{comboBox2.SelectedItem}');";
                try
                {
