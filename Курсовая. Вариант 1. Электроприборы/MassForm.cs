...
namespace Курсовая.Вариант_1.Электроприборы
{
    public partial class MassForm : Form
    {
        ...
        public MassForm(SqlConnection connection)
        {
            InitializeComponent();
...
JOIN [EA_type] t
ON t.[ID] = pl.[Type_ID]
where pl.[Mass] between {numericUpDown1.Value} and {numericUpDown2.Value};";
            }
            else
            {
                select = $@"select [Date of manufacture] ""Дата производства"", m.[Name] ""Производитель"", [Mass] ""Масса"",
[Price] ""Цена продажи"",  [Date of sale] ""Дата продажи"", 
c.[Name] ""Страна - производитель"", pl.[Product code]  ""Артикул"", t.[Name] ""Тип электроприбора"", ds.[Defective status] ""Товар бракован?"" 
from [Product list] pl
JOIN [Sales log] sl
...
JOIN [EA_type] t
ON t.[ID] = pl.[Type_ID]
where m.[Name]='{comboBox2.SelectedItem}' and
(pl.[Mass] between {numericUpDown1.Value} and {numericUpDown2.Value});";
            }
            try
            {
